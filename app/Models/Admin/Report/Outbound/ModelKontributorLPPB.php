...
class ModelKontributorLPPB extends Model
{
    use HasFactory;

    public static function getSummaryData($facilityID, $Relasi,  $StartDate, $EndDate, $Key1, $Key2, $Key3)
    {
        try {
            ...
            $query = "SET NOCOUNT ON; EXEC [Report_Kontributor_LPPB] ?,?,?,?,?,?,?,?";
    ...
    public static function getDeptName($facilityID, $Relasi,  $StartDate, $EndDate, $Key1, $Key2, $Key3)
    {
        try {
            $query = "SET NOCOUNT ON; EXEC [Report_Kontributor_LPPB] ?,?,?,?,?,?,?,?";
            return DB::select($query, ['DeptName', $facilityID, $Relasi,  $StartDate, $EndDate, $Key1, $Key2, $Key3]);
        } catch (\Exception $e) {
            throw new \Exception('Error executing stored procedure: ' . $e->getMessage());
        }
    }

    public static function getDetailData($facilityID, $Relasi,  $StartDate, $EndDate, $Key1, $Key2, $Key3)
    {
        try {
            set_time_limit(300);
