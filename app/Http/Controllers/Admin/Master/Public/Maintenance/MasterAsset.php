...
use App\Models\Admin\Master\Public\Maintenance\ModelMasterAsset;
...
class MasterAsset extends Controller
{
    ...
    public function ViewPageMasterAsset(Request $request)
    {
        try {
            ...
            $Owner = '';
            $dataTable = ModelMasterAsset::getTypeUnit();

            $dataTable1 = ModelMasterAsset::getDataView($facilityID, $Relasi, $Owner);
            ...
            return view('admin.master.public.Maintenance.MasterAsset', compact('dataTable', 'dataowner5r', 'dataTable1'))
                ->with($notification);
        } catch (\Exception $e) {
            Log::error('Error logging user view: ' . $e->getMessage());
            return back()->with('error', 'Failed to log page view.');
    ...
    public function GetData(Request $request)
    {
        try {
            ...
            $result = ModelMasterAsset::getData($facilityID, $Relasi, $Owner, $AssetNo, $UnitNo);
            //dd($result);
            if (!$result) {
    ...
    public function SaveData(Request $request)
    {
        try {
            ...
            //dd($facilityID, $Relasi, $Owner, $AssetNo, $UnitNo, $UnitName, $UnitType, $ReceiveDate, $LifeTime, $IsActive, $AddUser, $Brand);
            ModelMasterAsset::insertDataView($facilityID, $Relasi, $Owner, $AssetNo, $UnitNo, $UnitName, $UnitType, $ReceiveDate, $LifeTime, $IsActive, $AddUser, $Brand);
    ...
    public function deleteData(Request $request)
    {
        try {
            ...
            //dd($facilityID, $Relasi, $Owner, $AssetNo, $UnitNo, $UnitName, $UnitType, $ReceiveDate, $LifeTime, $IsActive, $AddUser, $Brand);
            ModelMasterAsset::deleteDataView($facilityID, $Relasi, $Owner, $AssetNo, $UnitNo, $UnitName, $UnitType, $ReceiveDate, $LifeTime, $IsActive, $AddUser, $Brand);
            return back()->with($notification);
        } catch (\Exception $e) {
            Log::error('Error logging user view: ' . $e->getMessage());
            return back()->with('error', 'Failed Insert Data, Area  Harus Huruf dan Angka.');
