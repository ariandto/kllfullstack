...
use App\Models\Admin\Report\Inventory\ModelReportIntercompany;
...
class ReportIntercompany extends Controller
{
    public function ViewPageReportIntercompany(Request $request)
    {
        try {
            ...
            $tableHeaders1 = [];
            // Kirim data ke view
            return view('admin.report.inventory.report_intercompany');
        } catch (\Exception $e) {
    ...
    public function ViewPageSummaryReportIntercompany(Request $request)
    {
        try {
            ...
            $dataTables = ModelReportIntercompany::getSummaryData($facilityID, $Relasi, $StartDate, $EndDate,  $Key1, $Key2);
            // Kirim data ke view
            ...
            return redirect()->route('admin.report.inventory.summary_reportintercompany')
                ->with([
    ...
    public function ViewPageDetailReportIntercompany(Request $request)
    {
        try {
            $facilityInfo = session('facility_info', []);
            $facilityID = $facilityInfo[0]['Facility_ID'] ?? $request->input('WHSEID', '');
            ...
            $dataTables = ModelReportIntercompany::getDetailData($facilityID, $Relasi, $StartDate, $EndDate, $Key1, $Key2);
            // dd($dataTables);
    ...
    public function InsertCommentReportIntercompany(Request $request)
    {
        try {
            $id = Auth::guard('admin')->id();
            $data = Admin::find($id);
            ...
            ModelReportIntercompany::insertDataComments($facilityID, $Relasi, $Tanggal, $Comment, $Addwho);
            return response()->json([
                'success' => true,
