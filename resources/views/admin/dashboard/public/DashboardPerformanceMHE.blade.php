...
@section('admin')
    <div class="page-content">
        <div class="container-fluid">
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form id="reportForm" method="POST"
                                action="{{ route('admin.dashboard.public.dashboardperformancemhe') }}">
                                @csrf
                                <div class="row">
                            ...
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-center">
                                    <div>
                                        ...
                                        <h3 class="mb-0">
                                            <span class="counter-value" data-target="{{ $summary['Total'] ?? 0 }}">0</span>
                                        </h3>
                                    </div>
                                            ...
                                            <tr>
                                                @foreach ($tableHeaders2 as $header)
                                                    @if (in_array($header, [
                                                            ...
                                                            'Status',
                                                            'Terakhir Preventive',
                                                            'Jadwal Preventive Berikutnya',
                                                            'Total Breakdown Bulanan',
                                                        ]))
                                                        @php
                                                            $thClass = match ($header) {
                                                                ...
                                                                'Unit Type' => 'unit-type',
                                                                'Status' => 'status',
                                                                'Terakhir Preventive' => 'Terakhir-Preventive',
                                                                'Jadwal Preventive Berikutnya'
                                                                    => 'Jadwal-Preventive-Berikutnya',
                                                                'Total Breakdown Bulanan' => 'Total-Breakdown-Bulanan',
                                                                default => '',
                                                            };
                                                        @endphp
            ...
            <div class="card mt-4">
                <div class="card-body d-flex" style="gap: 30px;">
                    <div style="flex: 1;">
                        <h5 class="card-title">Grafik Breakdown Bulanan</h5>
                        <canvas id="lineChartBreakdown" height="100"></canvas>
                    </div>

                    <div style="flex: 1;">
                        <h5 class="card-title">Grafik Mean Time To Repair Bulanan</h5>
                        <canvas id="barChartMTTR" height="100"></canvas>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        function closeCheckboxList(type) {
            if (type === 'dcname') {
                ...
                $.ajax({
                    url: "{{ route('admin.dashboard.public.get_unit_type_asset') }}",
                    type: "POST",
                    data: {
    ...
    <script>
        // Register plugin datalabels sekali saja
        Chart.register(ChartDataLabels);
    </script>

    @if (!empty($dataTable3))
        <script>
            // Chart Line Breakdown Bulanan
            const ctxLine = document.getElementById('lineChartBreakdown').getContext('2d');
            const lineChart = new Chart(ctxLine, {
                type: 'line',
                data: {
                    labels: {!! json_encode(array_column($dataTable3, 'Periode')) !!},
                    datasets: [{
                        label: 'Total Breakdown',
                        data: {!! json_encode(array_column($dataTable3, 'Total Breakdown')) !!},
    ...
    @if (!empty($dataTable4))
        <script>
            // Chart Bar MTTR Bulanan
            const ctxBar = document.getElementById('barChartMTTR').getContext('2d');
            const barChart = new Chart(ctxBar, {
                type: 'bar',
                data: {
                    labels: {!! json_encode(array_column($dataTable4, 'Periode')) !!},
                    datasets: [{
                        label: 'MTTR Days',
                        data: {!! json_encode(array_column($dataTable4, 'MTTR Days')) !!},
    ...
    <style>
        ...
        /* Tambahan: atur lebar kolom tabel berdasarkan class */
        th.picture,
        td.picture {
            width: 60px;
        }
        ...
        th.Terakhir-Preventive,
        td.Terakhir-Preventive {
            width: 100px;
        }

        th.Jadwal-Preventive-Berikutnya,
        td.Jadwal-Preventive-Berikutnya {
            width: 120px;
        }

        th.Total-Breakdown-Bulanan,
        td.Total-Breakdown-Bulanan {
            width: 150px;
        }
